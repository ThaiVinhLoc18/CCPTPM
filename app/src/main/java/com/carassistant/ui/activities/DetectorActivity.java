...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
    ...
    private static float MINIMUM_CONFIDENCE_TF_OD_API = 0.9f;
    ...
    private void setupViews() {
        ...
        findViewById(R.id.privacyPolicy).setOnClickListener(v -> {
            Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.facebook.com/profile.php?id=100024627910057"));
    ...
    private SignEntity getSignImage(Classifier.Recognition result, Bitmap bitmap) {
        ...
        if ("Người đi bộ".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.crosswalk, R.raw.crosswalk);
        } else if ("Dừng".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.stop, R.raw.dung);
        } else if ("Đường ưu tiên".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.main_road, R.raw.duonguutien);
        ...
        } else if ("Cấm dừng".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.dont_stop, R.raw.camdung);
        } else if ("Cấm đậu".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.no_parking, R.raw.no_parking);
        } else if ("Đường cấm".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.dont_move, R.raw.duongcam);
        } else if ("Cấm đi ngược chiều".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.dont_enter, R.raw.camdinguoc);
        ...
        } else if ("Tối đa 40".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.speed_limit_40, R.raw.toida40);
        } else if ("Tối đa 50".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.speed_limit_50, R.raw.toida50);
        } else if ("Tối đa 60".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.speed_limit_60, R.raw.toida60);
        } else if ("Tối đa 70".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.speed_limit_70, R.raw.toida70);
        } else if ("Tối đa 80".equals(result.getTitle())) {
            sign = new SignEntity(result.getTitle(), R.drawable.speed_limit_80, R.raw.toida80);
